...
VSCode 是一个开发利器，通过安装插件可以轻松进行 Js Python等多种语言的开发，但安装 Go 的一些插件时总是提示失败，最初参照网上的一些方法解决了，最近把 go 版本升到了 1.13，VSCode 提示插件需要升级，这一升级不要紧，又安装失败了，错误和之前的还不一样，经过排查日志最后总算解决了，记录一下解决办法。
